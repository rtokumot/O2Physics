...
struct myLambda {
  ...
  Configurable<float> rapidity{
    "rapidity", 0.80,
    "V0 rapidity"};
  Configurable<float> tpcNcl{
    "tpcNcl", 70,
    "tpcNcl"};
  Configurable<float> dcanegtopv{
    "dcanegtopv", 0.05,
    "DCA Neg To PV"};
  Configurable<float> dcapostopv{
    "dcapostopv", 0.05,
    "DCA Pos To PV"};
  Configurable<float> tpcsigma{
    "tpcsigma", 5,
    "tpcsigma"};

  // V0 cuts
  Configurable<float> minpt{
    "minpt", 0.30,
    "minpt"};
  Configurable<float> v0radius{
    "v0radius", 0.2,
    "v0radius"};
  Configurable<float> dcav0dau{
    "dcav0dau", 1.5,
    "DCA Pos To PV"};
  Configurable<float> v0cospa{
    "v0cospa", 0.99,
    "V0 CosPA"};
  Configurable<float> removeKs{
    "removeKs", 0.025,
    "removeKs"};
...
struct myXi {
  ...
  Configurable<float> rapidity{
    "rapidity", 0.80,
    "V0 rapidity"};
  Configurable<float> mincrossedrow{
    "mincrossedrow", 70,
    "mincrossedrow"};
  Configurable<float> minpt{
    "minpt", 0.30,
    "minpt"};
  Configurable<float> dcatopv{
    "dcatopv", 0.05,
    "DCA To PV"};
  Configurable<float> tpcsigma{
    "tpcsigma", 4.0,
  ...
  Configurable<float> dcav0dau{
    "dcav0dau", 1.6,
    "DCA Pos To PV"};
  Configurable<float> dcav0topv{
    "dcav0topv", 0.07,
    "dcav0topv"};
  Configurable<float> v0cospa{
    "v0cospa", 0.97,
    "v0CPA"};
  Configurable<float> v0masswindow{
    "v0masswindow", 0.006,
    "v0masswindow"};

  // casc cuts
  Configurable<float> cascradius{
    "cascradius", 0.8,
    "cascradius"};
  Configurable<float> dcacascdau{
    "dcascascdau", 1.6,
    "DCA V0 to bach"};
  Configurable<float> casccospa{
    "casccospa", 0.98,
    "casc CosPA"};
  Configurable<float> removeOmega{
    "removeXi", 0.005,
    "removeXi"};
  ...
  void process(aod::Collision const& collision,
               ...
               aod::V0sLinked const& V0linked, aod::V0Datas const& V0s,
               DauTracks const& tracks, aod::McParticles const& mcParticles)
  {
    for (auto& casc : Cascades) {
      ...
      if ( // Dau & bach track cuts
        ...
        if (TMath::Abs(v0.mLambda() - RecoDecay::getMassPDG(kLambda0)) <
              v0masswindow &&

            // Cascade cut
            casc.cascradius() > cascradius &&
            casc.dcacascdaughters() < dcacascdau &&
            casc.casccosPA(collision.posX(), collision.posY(),
                           collision.posZ()) > casccospa &&
            TMath::Abs(casc.mOmega() - RecoDecay::getMassPDG(kOmegaMinus)) >
              removeOmega &&
            casc.sign() < 0) {
          registry.fill(HIST(
                          "hReduction"),
...
struct myOmega {
  ...
  Configurable<float> casccospa{
    "casccospa", 0.995,
    "casc CosPA"};
  Configurable<float> removeXi{
    "removeXi", 0.005,
    "removeXi"};

  using DauTracks = soa::Join<aod::Tracks, aod::TracksExtra, aod::pidTPCPi,
    ...
    for (auto& casc : Cascades) {
      ...
      if ( // Dau & bach track cuts
        ...
        if (TMath::Abs(v0.mLambda() - RecoDecay::getMassPDG(kLambda0)) <
            ...
            casc.casccosPA(collision.posX(), collision.posY(),
                           collision.posZ()) > casccospa &&
            TMath::Abs(casc.mXi() - RecoDecay::getMassPDG(kXiMinus)) >
              removeXi &&
            casc.sign() < 0) {
          registry.fill(HIST(
                          "hReduction"),
